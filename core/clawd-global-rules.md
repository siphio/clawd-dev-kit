...
## MANDATORY: Generate docs/CLAUDE.md

**Every capability project MUST have a `docs/CLAUDE.md` file.** This file is auto-loaded by Claude Code every session and provides persistent guidance for Clawdbot/Moltbot capability development.

### When to Generate

- During `/clawd-create-prd` (project initialization)
- When setting up a new capability folder manually
- If `docs/CLAUDE.md` is missing from an existing project
...
⚠️ **CRITICAL**: Clawdbot/Moltbot capabilities are **prompt-orchestration first**, NOT code/tool-calling centric.
...
- **Invocation Patterns**:
  ...
  - Explicit exec/shell/curl: "Use exec to run python /skills/script.py --args" or "curl -X POST ...".
  - Step-by-step natural language: Numbered procedures that explicitly use built-ins.
- **Code is Optional Helper Only**: Scripts in skills/ folders are run ONLY when SKILL.md explicitly tells the agent via exec. No auto-discovery or index.ts tool exposure.
...
- Using `/v1/chat/completions` for APIs that have moved to new endpoints.
...
- Flag any references to deprecated endpoints.
...
# Replace capability name placeholder
CAPABILITY_NAME="${1:-$(basename $(pwd))}"
...
After generation, verify the file exists and is under the size limit:

```bash
if [ -f ./docs/CLAUDE.md ]; then
    LINES=$(wc -l < ./docs/CLAUDE.md)
    echo "✅ docs/CLAUDE.md exists ($LINES lines)"
    if [ $LINES -gt 800 ]; then
        echo "⚠️  Warning: CLAUDE.md exceeds 800 lines. Consider condensing."
    fi
else
    echo "❌ docs/CLAUDE.md missing - run generation command above"
fi
```
